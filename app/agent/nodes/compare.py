...
@log_node_execution("enhance_compare_sql")
async def enhance_compare_sql(state: DataAgentState, runtime: Runtime[DataAgentContext]):
    """增强对比分析的SQL生成"""
    writer = runtime.stream_writer
    writer({"type": "progress", "step": "对比分析增强", "status": "running"})
    ...
    try:
        ...
        # 只有对比分析才需要增强
        if intent != "compare":
            writer({"type": "progress", "step": "对比分析增强", "status": "skip"})
            return {}
        ...
        prompt = PromptTemplate(
            template=load_prompt("generate_compare_sql"),
            input_variables=[
                "query",
                "table_columns",
                "metric_infos",
        ...
        writer({"type": "progress", "step": "对比分析增强", "status": "success"})
        logger.info(f"[{trace_id}] 对比分析SQL生成成功: {sql}")
        return {"sql": sql}

    except Exception as e:
        writer({"type": "progress", "step": "对比分析增强", "status": "error"})
        logger.error(f"对比分析增强失败: {str(e)}")
        raise
