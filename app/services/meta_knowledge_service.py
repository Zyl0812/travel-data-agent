...
class MetaKnowledgeService:
    ...
    async def build(self, meta_config_path: Path):
        ...
        if meta_config.tables:
            # 2.1 保存表格信息和字段信息到meta数据库，返回字段信息列表
            column_infos: list[ColumnInfo] = await self._save_table_info_to_meta_db(
                meta_config
            )
            ...
            await self._save_values_to_es(meta_config, column_infos)
            logger.info("为字段取值建立全文索引成功")

        # 3. 核心-处理指标（包含字段指标关系）信息
        if meta_config.metrics:
            # 3.1 保存指标（包含字段指标关系）信息到meta数据库
            metric_infos: list[MetricInfo] = await self._save_metric_info_to_meta_db(
                meta_config
            )
    ...
    async def _save_table_info_to_meta_db(
        ...
        # 1. 初始化存放表信息 dataclass 列表，字段信息 dataclass 列表
        ...
        for table in meta_config.tables:
            ...
            # 3. 遍历表信息包含的字段列表
            # 3.1 调用数仓持久层获取指定表中的每个字段类型
            column_type_dict: dict[
                str, str
            ] = await self.dw_mysql_repository.get_column_types(table.name)

            for column in table.columns:
                # 3.2 调用数仓持久层获取指定表中字段部分取值
                values: list[str] = await self.dw_mysql_repository.get_column_values(
    ...
    async def _save_column_info_to_qdrant(self, column_infos: list[ColumnInfo]):
        ...
        # 2. 准备持久层向量数据点需要的三项数据：id, embedding_text, payload
        points = []
        ...
        for column_info in column_infos:
            ...
            if column_name in column2es:
                ...
                values = await self.dw_mysql_repository.get_column_values(
                    table_name, column_name, limit=100000
                )
                for value in values:
                    ...
                    values_infos.append(value_info)
